...
- [x] Task 1: 의존성 설치 및 환경변수 설정 (AC: #1)
  - [x] `bun add @anthropic-ai/sdk@0.82.0 zod@4.3.6`
  - [x] `.env.local`에 `ANTHROPIC_API_KEY` 추가
  - [x] `.env.local.example`에 템플릿 추가
  - [x] Vercel Environment Variables에 등록
- [x] Task 2: LandingPageData Zod 스키마 작성 (AC: #1, #2)
  - [x] `src/lib/schemas/landing.ts` 생성
  - [x] hero, valueProps(3개), cta, emailForm 스키마 정의
  - [x] `export type LandingPageData = z.infer<typeof LandingPageData>`
- [x] Task 3: generateLanding 함수 작성 (AC: #3, #4, #5, #6)
  - [x] `src/lib/llm/generate.ts` 생성
  - [x] `claude-sonnet-4-6` 모델 사용
  - [x] tool_use 방식 structured output
  - [x] `LandingPageData.parse(toolUse.input)` 검증
  - [x] Kill Switch 없음 (Story 1.7에서 추가)
- [x] Task 4: 테스트 호출 확인 (AC: #7)
  - [x] `src/app/api/llm-test/route.ts` 생성
  - [x] "퇴근 후 15분 코딩 챌린지" 프롬프트로 호출 성공
  - [x] hero, valueProps(3개), cta, emailForm 모두 정상 반환 확인
...
- `bun add @anthropic-ai/sdk zod` → @anthropic-ai/sdk@0.82.0, zod@4.3.6 설치
- `/api/llm-test` 로컬 테스트 성공 — "퇴근 후 15분 코딩 챌린지" 프롬프트로 유효한 LandingPageData 반환

### Completion Notes List

- **모델 ID**: Story Dev Notes의 `claude-sonnet-4-5-20250929` 대신 최신 `claude-sonnet-4-6` 사용
- **Anthropic 초기화 위치**: `new Anthropic()` 호출을 함수 내부에 배치 — 모듈 로드 시점이 아닌 요청 시점에 초기화. DB client lazy init 패턴과 동일.
- **Zod v4**: zod@4.3.6 설치됨. API는 v3와 동일.

### File List

**생성:**
- `src/lib/schemas/landing.ts` — LandingPageData Zod 스키마 (Single Source of Truth)
- `src/lib/llm/generate.ts` — generateLanding 함수 (Anthropic tool_use)
- `src/app/api/llm-test/route.ts` — 임시 테스트 route

**수정:**
- `package.json` — @anthropic-ai/sdk, zod 추가
- `bun.lock` — 업데이트
- `.env.local` — ANTHROPIC_API_KEY 추가 (gitignored)
- `.env.local.example` — ANTHROPIC_API_KEY 템플릿 추가
