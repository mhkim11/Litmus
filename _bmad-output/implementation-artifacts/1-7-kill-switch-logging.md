...
- [x] Task 1: 환경변수 설정 (AC: #1)
  - [x] `.env.local`에 `MAX_MONTHLY_USD=50`, `KILL_SWITCH_OVERRIDE=false` 추가
  - [x] `.env.local.example`에 템플릿 추가
  - [x] Vercel Environment Variables에 등록
- [x] Task 2: budget.ts 순수 함수 작성 (AC: #1)
  - [x] `src/lib/llm/budget.ts` 생성
  - [x] `isBudgetExceeded(currentMonthUsage, maxBudget, overrideEnabled)` 구현
- [x] Task 3: BudgetExceededError 클래스 정의 (AC: #5)
  - [x] `src/lib/llm/gateway.ts`에 정의
- [x] Task 4: logLLMCall 쿼리 함수 (AC: #3)
  - [x] `src/lib/db/queries/llmCalls.ts` 생성, Drizzle INSERT 구현
- [x] Task 5: getCurrentMonthLLMCost 쿼리 함수 (AC: #2)
  - [x] `llmCalls.ts`에 추가, date_trunc 기반 월 합산
- [x] Task 6: checkAndEnforceBudget 통합 함수 (AC: #2, #4)
  - [x] `src/lib/llm/gateway.ts`에 구현, export
- [x] Task 7: budget.test.ts 유닛 테스트 (AC: #6)
  - [x] vitest@4.1.2 설치
  - [x] 3/3 pass (under budget, over budget, override enabled)
- [x] Task 8: 통합 테스트 (AC: #7)
  - [x] Neon에 cost_usd=55 row 삽입
  - [x] `/api/budget-test` → `{"ok":false,"error":"LLM monthly budget exceeded: $55.00 / $50.00"}` 확인
  - [x] 테스트 row 삭제 완료
...
- vitest 3/3 pass: under budget(false), over budget(true), override(false)
- `/api/budget-test`: `{"ok":false,"error":"LLM monthly budget exceeded: $55.00 / $50.00","usage":55,"max":50}`

### Completion Notes List

- **Drizzle lazy init**: `drizzle(getDb())`를 함수 내부에서 호출 — 모듈 로드 시 DB 초기화 방지 (DB client lazy init 패턴 일관성)
- **numeric 타입 → string**: Drizzle의 numeric 컬럼은 TypeScript에서 string 타입. promptTokens, completionTokens, costUsd, durationMs 모두 String() 변환 후 INSERT
- **generateLanding 미수정**: AC #4 준수 — Story 2.4에서 통합

### File List

**생성:**
- `src/lib/llm/budget.ts` — isBudgetExceeded 순수 함수
- `src/lib/llm/budget.test.ts` — vitest 유닛 테스트
- `src/lib/llm/gateway.ts` — BudgetExceededError + checkAndEnforceBudget
- `src/lib/db/queries/llmCalls.ts` — logLLMCall + getCurrentMonthLLMCost
- `src/app/api/budget-test/route.ts` — 임시 통합 테스트 route

**수정:**
- `.env.local` — MAX_MONTHLY_USD, KILL_SWITCH_OVERRIDE 추가
- `.env.local.example` — 템플릿 추가
- `package.json` / `bun.lock` — vitest 추가
