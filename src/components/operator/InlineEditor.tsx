...
function EditableField({
  ...
  return (
    <Tag
      ...
      className={`outline-none border-b border-dashed border-gray-300 dark:border-gray-600 focus:border-blue-400 cursor-text ${className ?? ''}`}
    >
      {value}
    </Tag>
  )
...
export default function InlineEditor({ ideaId, pageData, onChange }: InlineEditorProps) {
  ...
  return (
    <div className="border border-blue-200 dark:border-blue-800 rounded-lg p-6 bg-blue-50 dark:bg-blue-950 flex flex-col gap-6">
      <div className="flex items-center justify-between">
        <p className="text-xs text-blue-600 dark:text-blue-400 font-medium">직접 편집 모드 — 필드를 클릭해 수정하세요</p>
        <span className={`text-xs ${saveStatus === 'error' ? 'text-red-500' : 'text-gray-400 dark:text-gray-500'}`}>
          {saveStatus === 'saving' ? '저장 중...' : saveStatus === 'saved' ? '저장됨' : saveStatus === 'error' ? '❌ 저장 실패' : ''}
        </span>
      </div>

      <section>
        <p className="text-xs text-gray-400 dark:text-gray-500 uppercase tracking-wide mb-2">Hero</p>
        <EditableField
          tag="p"
      ...
      <section>
        <p className="text-xs text-gray-400 dark:text-gray-500 uppercase tracking-wide mb-2">CTA</p>
        <EditableField
          tag="p"
