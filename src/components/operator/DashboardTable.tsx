...
function SortLink({
  ...
  return (
    <a
      ...
      className={`hover:underline cursor-pointer select-none ${isActive ? 'text-blue-600 dark:text-blue-400' : ''}`}
    >
      {label}{arrow}
    </a>
  )
      ...
      <div className="overflow-x-auto rounded-lg border border-gray-200 dark:border-gray-700">
        <table className="w-full text-sm">
          <thead className="bg-gray-50 dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700">
            <tr>
              <th className="text-left px-4 py-3 font-medium text-gray-600 dark:text-gray-300">프롬프트</th>
              <th className="text-left px-4 py-3 font-medium text-gray-600 dark:text-gray-300 w-20">상태</th>
              <th className="text-center px-4 py-3 font-medium text-gray-600 dark:text-gray-300 w-16">
              ...
              <th className="text-center px-4 py-3 font-medium text-gray-600 dark:text-gray-300 w-20">CTA율</th>
              <th className="text-center px-4 py-3 font-medium text-gray-600 dark:text-gray-300 w-20">이메일율</th>
              <th className="text-left px-4 py-3 font-medium text-gray-600 dark:text-gray-300 w-24">발행 URL</th>
          ...
          <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
            {ideas.map((idea) => (
              <tr key={idea.id} className="hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
                <td
                  className="px-4 py-3 text-gray-900 dark:text-gray-100 cursor-pointer"
                  onClick={() => { window.location.href = `/operator/ideas/${idea.id}` }}
                >
                  {idea.finalPrompt
                    ? idea.finalPrompt.slice(0, 50) + (idea.finalPrompt.length > 50 ? '...' : '')
                    : <span className="text-gray-400 dark:text-gray-500 italic">빈 프롬프트</span>}
                ...
                <td className="px-4 py-3 text-center text-gray-700 dark:text-gray-300">{fmt(idea.pv)}</td>
                <td className="px-4 py-3 text-center text-gray-700 dark:text-gray-300">{fmt(idea.ctaClicks)}</td>
